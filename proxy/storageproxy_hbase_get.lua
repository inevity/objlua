...
function _M.SendtoHbase(self, API_uri, op_ob, headers, body)
    ...
    -- if "service" == op then
    -- end
    --注：如果有body，放入http连接前要用json.encode编码
    ...
    if nil == res then
		...
		if not ok or type(jbody)~="table" then
	    	ngx.log(ngx.ERR, "Response body error when invoke json.decode. body is ", tostring(body), "; when send HTTP_GET_Request to hbase")
